...
# simpris imports
from simpris.api.ideamap.serializers import IdeaMapSerializer
from simpris.library.user_context import UserContextFull
from simpris.models.models import Idea, IdeaMap

logger = logging.getLogger(__name__)


# serializer for ideas
class APIIdeaMapSerializer(serializers.ModelSerializer):
    class Meta:
        model = IdeaMap
        fields = ('id', 'clientid', 'name', 'description', 'updateddate', 'updatedby')


@api_view(['GET'])
def list(request):
    user_context = UserContextFull(request)
    logger.info(str.format('idea map list : {0}' .format(user_context.id)))

    proj_id = request.query_params.get('proj_id')

    queryset = IdeaMap.objects.all().filter(Q(clientid=user_context.clientID) & Q(parentid=proj_id) & Q(deleteddate__isnull=True))\
        .order_by('id')
    serializer_class = IdeaMapSerializer(queryset, many=True)
...
def insert(request):
    ...
    user_context = UserContextFull(request)
    logger.info(str.format('idea map create : {0}' .format(user_context.id)))
    ...
    idea_serializer = APIIdeaMapSerializer(data={'id': 0, 'name': idea_name, 'description': idea_description,
    ...
    if idea_serializer.is_valid():
        ...
        idea_object = IdeaMap.objects.filter(clientid=user_context.clientID, createdby=created_by).latest('id')
...
def delete(request):
    ...
    user_context = UserContextFull(request)
    logger.info(str.format('idea map delete : {0}' .format(user_context.id)))
    ...
    try:
        idea_update = IdeaMap.objects.get(name=idea_name, clientid=user_context.clientID)
...
def update(request):
    ...
    user_context = UserContextFull(request)
    logger.info(str.format('idea map update : {0}' .format(user_context.id)))
    ...
    idea_serializer = APIIdeaMapSerializer(data={'id': 0, 'clientid': user_context.clientID, 'name': idea_name,
    ...
    if idea_serializer.is_valid():
        ...
        idea_update = IdeaMap.objects.get(clientid=user_context.clientID, name=idea_name)
